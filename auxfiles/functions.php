               ...
               if ($arr['error']['mysql_err_no'] == 0) {
                  while ($row = mysql_fetch_array($result)) {
                     ...
                     while ($i < mysql_num_fields($result)) {
                        ...
                        $type = $meta->type;
                        $arr['fields'][$field_name][] = mysql_change_utf8($row[$field_name],$type);
                        $i++;
                     }
                  }
...
function mysql_change_utf8 ($str,$type) {
   $ret = $str;
   $field_types = array ('string','blob');
   //debug('type----->',$type);debug('encoding----->',$str." - ".mb_detect_encoding($str));
   if (in_array(strtolower($type),$field_types) && strtolower(mb_detect_encoding($str)) == 'utf-8') {
      $ret = utf8_decode($str);
   }
